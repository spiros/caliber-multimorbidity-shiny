...
#  titlePanel("Whole Disease Network"),
  ...
  titlePanel("Multimorbidity Network App"),
...
#    selectInput('dz', 'Index Disease', sort(node_asian_whole_2010$Disease), selected="Macular Degeneration"), 
    ...
    numericInput('cocut', 'Partial Correlation cut-off', value=0.1, step=0.05),
...
#    selectInput('layout', 'Layout',c("layout_as_star","layout_with_fr"), selected="layout_as_star"), 
    ...
    numericInput('height', 'Height', value=1000, step=100),
    numericInput('width', 'Width', value=1000, step=100),
...
fluidRow(column(12, div(dataTableOutput("dataTable")))) ,
	...
	out = h5("Infections were grouped by causal organism and organ system according to the dual-axis ICD-10 classification system and could therefore be more highly represented in multimorbidity networks. This should be taken into account when interpreting the multimorbidity networks.")
  ...
  mainPanel(
     tabsetPanel(
        ...
        tabPanel("User guide",
        h5(strong("MULTIMORBIDITY NETWORKS")),
        h5(p("Networks are by definition composed of nodes (also called points) and edges (also called links or lines). In multimorbidity networks, nodes represent health conditions and edges (the connections between nodes) represent the strength of association between the linked health conditions (based on the partial correlation). An edge is present between two nodes if the partial correlation is above a specified threshold. The width of the edge is proportional to the partial correlation between the health conditions. The size of a node is proportional to the period prevalence of the health condition between 1 April 2010 and 31 March 2015 in the sample population from which the network is drawn.")),
#        br(),        
...
#        h5("Table. Descriptive statistics of partial correlation values for subpopulation stratified by sex, ethnicity and age."),
        br(),
        h5(p("As a guide, the 90th percentile for the partial correlation in the whole population is 0.011, while the 99th percentile is 0.085. The default for the partial correlation cut-off has been set at 0.1 as this value allows for a clear visualisation of the multimorbidity networks with a meaningful number of connections for most stratified subgroups.")),
#        br(),  
