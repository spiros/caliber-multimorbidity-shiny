...
inputdzlab<-reactive({rownames(node_2010()[node_2010()$Disease==input$dz,]) })

linked.dz<-reactive({
unique(as.vector(as.matrix(select(filter(link_2010()[DzA %in% inputdzlab()|DzB %in% inputdzlab(),],partialcor.pval<input$pcut) %>% filter(partialcor>input$cocut), DzA:DzB))))
  })

dz.link<-reactive({
filter(link_2010()[DzA %in% linked.dz() & DzB %in% linked.dz(),],partialcor.pval<input$pcut) %>% filter(partialcor>input$cocut)
  })
